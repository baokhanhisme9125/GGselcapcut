...
async function verifyOrder(invoiceId) {
  ...
  // Log full raw data to find correct field names
  console.log(`[ggsel-raw] Full purchase data for invoice ${invoiceId}:`, JSON.stringify(purchase));

  // Brute-force: search ALL fields (including nested) for product ID
  function findField(obj, prefix = '') {
    const results = {};
    for (const [k, v] of Object.entries(obj || {})) {
      const path = prefix ? `${prefix}.${k}` : k;
      if (typeof v === 'object' && v !== null && !Array.isArray(v)) {
        Object.assign(results, findField(v, path));
      } else {
        results[path] = v;
      }
    }
  ...
  const allFields = findField(purchase);
  console.log(`[ggsel-fields] All fields:`, JSON.stringify(allFields));

  return {
    buyerEmail,
    datePay: purchase.date_pay || purchase.pay_date || new Date().toISOString(),
    isPaid:  purchase.invoice_state === 3,
    productId: String(purchase.id_goods || purchase.product_id || purchase.goods_id || ''),
    productName: purchase.product_name || purchase.name_goods || purchase.goods_name || '',
    raw:     purchase,
  };
}
